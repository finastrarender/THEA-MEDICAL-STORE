export const ValidationRules = {
  ...
  facility: {
    ...
    max: "Facility name cannot exceed 100 characters.",
...
export function validateField(name: string, value: string): string | null {
  ...
  switch (name) {
    ...
    case "phone":
      ...
      if (!/^\+?[0-9]+$/.test(v)) return ValidationRules.phone.pattern;
      const digits = v.replace(/\+/g, "");
      if (digits.length < 8 || digits.length > 15) return ValidationRules.phone.digits;
      if (!v.startsWith("+")) return "Phone number must include a country code starting with '+'.";
      return null;
    case "facility":
      if (!v) return ValidationRules.facility.required;
      if (v.length < 2) return "Facility name must be at least 2 characters long.";
      if (v.length > 100) return ValidationRules.facility.max;
